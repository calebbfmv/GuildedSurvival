...
import us.iluthi.soulofw0lf.ultimatesurvival.inventories.EditArmor;
    ...
    public static void invClick (InventoryClickEvent event){
        ...
        if (event.getRawSlot() == 44){
         ...
         if (event.getRawSlot() == 0
                ...
                || event.getRawSlot() == 26){
            p.closeInventory();
            Maps.editArmor.put(p.getName(), new Armor(event.getCurrentItem()));
            p.openInventory(EditArmor.inv(p));
        }
    }
}
